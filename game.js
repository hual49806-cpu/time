(()=>{"use strict";
const $=id=>document.getElementById(id),screens=["home","select","map","battle","result","victory"];
const LEVELS=[
{name:"第一關：生字森林",cat:["生字詞"],bg:"level1.webp",reward:{coin:50,shard:10}},
{name:"第二關：詞語河流",cat:["語詞運用"],bg:"level2.webp",reward:{coin:70,shard:15}},
{name:"第三關：幸福記憶",cat:["課文理解","修辭句型"],bg:"level3.webp",reward:{coin:90,shard:20}},
{name:"第四關：時間裂隙",cat:["課文理解","修辭句型","素養情境"],bg:"level4.webp",reward:{coin:120,shard:30}}
];
...
const bank=(window.QUESTION_BANK&&window.QUESTION_BANK.questions)||[];
const show=id=>screens.forEach(s=>$(s).classList.toggle("active",s===id));
function save(){localStorage.setItem("timeRepairV2",JSON.stringify(state));$("continueGame").classList.remove("hidden")}
function load(){try{const x=JSON.parse(localStorage.getItem("timeRepairV2"));if(x)state={...state,...x}}catch(e){}}
